...
#include "flash.h"
#include "flash_fwd_fp16_sparse_kernel_v2.h"
...
DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_kernel_fp16_v2_map, bool Is_causal, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Record_approx, int Quant_mode, bool Two_pass) {
...
void run_flash_fwd_fp16_v2(Flash_fwd_params &params, cudaStream_t stream) {
            ...
            BOOL_SWITCH(recording, RecordApprox, [&] {
                QUANT_MODE_SWITCH(params.quant_mode, Quant_mode, [&] {
                    BOOL_SWITCH(params.two_pass, Two_pass, [&] {
                        auto kernel = &flash_fwd_kernel_fp16_v2_map<Sparse_Kernel_traits, true, IsEvenMNConst && IsEvenKConst && Sparse_Kernel_traits::kHeadDim <= 128, true, false, RecordApprox, Quant_mode, Two_pass>;
...
void run_mha_fwd_hdim128_fp16_v2_map(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    if (is_sm8x) {
        BLOCK_N_SWITCH(params.block_k_size, kBlockN, [&]{
            BLOCK_SEG_SWITCH(params.block_seg_size, kBlockSeg, [&]{
                run_flash_fwd_fp16_v2<
                    Fp16_Flash_fwd_kernel_traits_v1<Headdim, 64, kBlockN, 4, 128, kBlockSeg, false, false, T>, false, Is_causal, true
                >(params, stream);
            });
        });
    } else {
