...
template<typename Kernel_traits, bool Is_causal, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Extra_reserve_blocks, bool Record_approx, typename Params>
inline __device__ void compute_attn_1rowblock_skipping_v1(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    Tensor mSK = make_tensor(
        make_gmem_ptr(reinterpret_cast<Element *>(params.sk_ptr) // correct
    ...
    int32_t * sbuf_ptr = reinterpret_cast<int32_t *>((sQMK.data() + size(sQMK)).get());
    ...
    constexpr int n_off_band_local_steps = Kernel_traits::kNLocalBlocks - n_masking_steps;
    static_assert(n_off_band_local_steps > 0, "n_off_band_local_steps should be larger than 0.");
    bool sink_token_covered = (Kernel_traits::kNLocalBlocks >= n_block_max); /* sink token block has not been covered. */

    // local and sink. TODO: Other reserved blocks.
    ...
    */   
    ...
    if (Kernel_traits::Is_QQ_in_regs && Kernel_traits::Share_QQ_QK_smem) {
        flash::cp_async_wait<0>();
    ...
    for (int istage = 0; istage < (Kernel_traits::kPipeSize - 1); istage++){
        flash::copy<true, Is_even_K>(gmem_tiled_copy_Quant, tKgQK(_, _, _, 1 + istage), tKsQK(_, _, _, istage), tKVcKV, tKVpKV);
        flash::copy_vector_off_band(gmem_tiled_copy_K_Scale, tKgSK(_, _, 1 + istage), tKsSK(_, _, istage));
        flash::copy_vector_off_band(gmem_tiled_copy_QMK, tQgQMK(_, _, 1 + istage), tQsQMK(_, _, istage));
        cute::cp_async_fence(); 
    }
    ...
    int in_prefetch = kPipeSize;
    int istage_now = 0; 

    n_block = 1;
    int n_block_end = n_block_max - Kernel_traits::kNLocalBlocks; 
    Element threshold = static_cast<Element>(params.threshold);
    int big_cnt = 0;
    for (; n_block < n_block_end; n_block ++ ) {
        ...
        {
            flash::copy<true, Is_even_K>(gmem_tiled_copy_Quant, tKgQK(_, _, _, in_prefetch), tKsQK(_, _, _, istage_prefetch), tKVcKV, tKVpKV);
            flash::copy_vector_off_band(gmem_tiled_copy_K_Scale, tKgSK(_, _, in_prefetch), tKsSK(_, _, istage_prefetch));
            flash::copy_vector_off_band(gmem_tiled_copy_QMK, tQgQMK(_, _, in_prefetch), tQsQMK(_, _, istage_prefetch));
            cute::cp_async_fence(); 
        ...
        CUTE_STATIC_ASSERT_V(size<0>(dummy_weight_fp32) == Int<4>{}, "Should be 4...");
        #pragma unroll
        for (int mi = 0; mi < size<1>(dummy_weight_fp32); mi++){
            ...
            for (int ni = 0; ni < size<2>(dummy_weight_fp32); ni++){
                ...
                for (int vi = 0; vi < size<0>(dummy_weight_fp32); vi++){ 
                    const int frag_row = vi / size<1>(get<0>(dummy_weight_fp32.layout()));
                    ...
                    // NOTE: int, float, tuple<int, int> 
                    // cute::static_assert_log(Int<32>{}, dummy_weight_fp32.layout()(make_coord(frag_col, frag_row), mi, ni), dummy_weight_fp32.data(), make_coord(frag_col, frag_row));

                    // auto something = dummy_weight_fp32.data().get(); 

                    // valid codes
                    // dummy_weight_fp32(make_coord(frag_col, frag_row), mi, ni) =  static_cast<float>(t4rQMK(make_coord(frag_col, ni), _0{}, _0{})); // good
                    // dummy_weight_fp32(vi, mi, ni) = dummy_weight_fp32.data()[dummy_weight_fp32.layout()(make_coord(1, 1), Int<4>{}, Int<3>{})] * t4rSk(make_coord(frag_col, ni), _0{}, _0{}); // good
                    // dummy_weight_fp32(vi, mi, ni) = dummy_weight_fp32.data()[dummy_weight_fp32.layout()(make_coord(1, 1), Int<4>{}, Int<3>{})] * 3; // good，
                    // dummy_weight_fp32(vi, mi, ni) = dummy_weight_fp32(vi, mi, ni) * t4rSk(make_coord(frag_col, ni), _0{}, _0{}) * rSQ(vi / 2 + 2 * mi) + t4rQMK(make_coord(frag_col, ni), _0{}, _0{}); // good                    
                    // dummy_weight_fp32(make_coord(vi, mi, ni)) = dummy_weight_fp32(make_coord(vi, mi, ni)) * t4rSk(make_coord(frag_col, ni), _0{}, _0{}) * rSQ(vi / 2 + 2 * mi) + t4rQMK(make_coord(frag_col, ni), _0{}, _0{}); // good                    
                    dummy_weight_fp32(target_reg) = dummy_weight_fp32(target_reg) * t4rSk(target_SK) * rSQ(rSQ_idx) + t4rQMK(target_SK);

                    // misbehave code.
                    // dummy_weight_fp32(make_coord(make_coord(frag_col, frag_row), mi, ni)) = dummy_weight_fp32(make_coord(make_coord(frag_col, frag_row), mi, ni)) * t4rSk(make_coord(frag_col, ni), _0{}, _0{}) * rSQ(vi / 2 + 2 * mi) + t4rQMK(make_coord(frag_col, ni), _0{}, _0{}); // bad                
                    // dummy_weight_fp32.data()[dummy_weight_fp32.layout()(make_coord(frag_col, frag_row), mi, ni)] = dummy_weight_fp32.data()[dummy_weight_fp32.layout()(make_coord(1, 1), Int<4>{}, Int<3>{})] * 3; 
                    // dummy_weight_fp32(vi, mi, ni) = dummy_weight_fp32(make_coord(1, 1), 0, 2) * 3; // bad
                    // dummy_weight_fp32(vi, mi, ni) = dummy_weight_fp32(make_coord(1, 1), Int<4>{},Int<3>{}) * 3; // bad
                    // dummy_weight_fp32(vi, mi, ni) = dummy_weight_fp32.data()[dummy_weight_fp32.layout()(make_coord(frag_col, frag_row), mi, ni)] * 3; // bad
                    // dummy_weight_fp32(make_coord(frag_col, frag_row), mi, ni) = dummy_weight_fp32(3, 0, 1) * 3; // bad
                }
            }
        }    


        bool is_big_block = softmax.template is_big_block<Record_approx>(dummy_weight_fp32, params.scale_softmax_log2, params.threshold); 

        if (Record_approx) {
            auto tAgApprox_view = gmem_thr_copy_approx.retile_D(tAgApprox);
            cute::copy(gmem_tiled_copy_Approx, dummy_weight_fp32, tAgApprox_view(_, _, _, n_block));
        }

        int big_enough = __any_sync(0xffffffff, is_big_block);
        // static_assert(Kernel_traits::kNWarps * 4 <= kBlockN * sizeof(Element), "Smem space for all reduce is too small!!");    
        if (tid_is_leader) temp_storage(warp_idx) = big_enough; 
        __syncthreads(); 

        big_enough = temp_storage(tid_warp % kNWarps); 
        big_enough = __any_sync(0xffffffff, big_enough);
    ...
    // if (bidh == 0 && bidb == 0 && m_block == 100 && tidx == 0 ){
    //     printf("Record_approx %d, sparsity result: %d \n", Record_approx, big_cnt);
    // }
    // Epilogue 
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// NOTE: Main Enter.
template<typename Kernel_traits, bool Is_causal, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Record_approx, typename Params>
inline __device__ void compute_attn(const Params &params) {
    ...
    flash::compute_attn_1rowblock_skipping_v1<
        Kernel_traits, Is_causal, Is_even_MN, Is_even_K, Is_softcap, true, Record_approx
