...
def layer2device(idx, layer_cnt):
    ...
    rank = torch.device(f"cuda:{idx //step}")
    return rank
...
def Qwen2AttentionPatch_kv_retrieve_calibration(attn: Qwen2Attention, config, idx):
...
def Qwen2DecoderLayerPatch(layer: Qwen2DecoderLayer, config, layer_idx):
    ...
    if attn_patch == "original":
        ...
        Qwen2AttentionPatch_kv_retrieve_calibration(layer.self_attn, config, layer_idx)
    return layer.half()
...
def PPQwen2ModelPatch(model: Qwen2Model, config):
    ...
    if os.environ.get("WORLD_SIZE", "1") == "1": # Scattered model scenario
        ...
        for i in range(config.num_hidden_layers):
            model.layers[i] = Qwen2DecoderLayerPatch(model.layers[i], config, i)
...
def Qwen2ForCausalLMPatch(M: Qwen2ForCausalLM, config):
    ...
    M.model = PPQwen2ModelPatch(M.model, config)
