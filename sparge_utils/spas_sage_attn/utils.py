...
def triton_quant(
    ...
    x_int8 = tl.extra.cuda.libdevice.float2int_rn(x_fp32 / scale)
...
def get_quant_int8(x, x_mean, block_size):
    x = x.contiguous()
    B, H, N, D = x.shape
    nblock = (N + block_size - 1) // block_size  # Number of blocks per feature map
    x_quant = torch.empty(x.shape, device=x.device, dtype=torch.int8)
    x_scale = torch.empty((B, H, nblock), device=x.device, dtype=torch.float32)
    grid = (B, H, nblock)
    triton_quant[grid](x, x_mean, x_quant, x_scale, N=N, D=D, BS=block_size, fuse_mean=(True if x_mean is not None else False))
...
def get_block_map_and_quant(q, k, km=None, map=None, is_causal=False, BLKQ=64, BLKK=32):
    Headnum = q.size(1)
    q_int8, q_scale = get_quant_int8(q, None, BLKQ)
    k_int8, k_scale = get_quant_int8(k, km, BLKK)

    lut, valid_block_num = block_map_lut_triton(map)
    return lut, valid_block_num, q_int8, k_int8, q_scale, k_scale
...
def get_block_map_meansim_fuse_quant(q, k, km=None, is_causal=False, BLKQ=128, BLKK=64, simthreshd1=0.1, cdfthreshd=0.9, is_sparse=True, return_lut=False, attention_sink=False):
    ...
    if not return_lut:
        ...
        lut, valid_block_num = block_map_lut_triton(final_map)
        # print("final_map", final_map.to(torch.int)[:,:2], lut[:,:2], valid_block_num[:,:2])
